...
let id;
let nonExistingId = `123456123456123456123456`;
const validProduct = {
  name: "iPhone",
...
describe("Test Products APIs", () => {
  ...
  it("Should test that GET /products/:id with a valid id returns 200 and a valid body", async () => {
  ...
  it("Should test that GET /products/:id with an invalid id returns 404", async () => {
    const response = await client.get(`/products/${nonExistingId}`).expect(404);
  });
  ...
  it("Should test that DELETE /products/:id with an valid id returns 204", async () => {
    const response = await client.delete(`/products/${id}`).expect(204);
  });
});
